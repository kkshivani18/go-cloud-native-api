...
type GoCdkAwsStackProps struct {
	awscdk.StackProps
}

func NewGoCdkAwsStack(scope constructs.Construct, id string, props *GoCdkAwsStackProps) awscdk.Stack {
	...
	cloudWatchRole := awsiam.NewRole(stack, jsii.String("ApiGatewayCloudWatchRole"), &awsiam.RoleProps{
		AssumedBy: awsiam.NewServicePrincipal(jsii.String("apigateway.amazonaws.com"), nil),
		ManagedPolicies: &[]awsiam.IManagedPolicy{
			awsiam.ManagedPolicy_FromAwsManagedPolicyName(jsii.String("service-role/AmazonAPIGatewayPushToCloudWatchLogs")),
		},
	})

	account := awsapigateway.NewCfnAccount(stack, jsii.String("ApiGatewayAccount"), &awsapigateway.CfnAccountProps{
...
func main() {
	...
	NewGoCdkAwsStack(app, "GoCdkAwsStack", &GoCdkAwsStackProps{
		awscdk.StackProps{
			Env: env(),
		},
	})
